...
class FirecrawlGatewayStack(Stack):

    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        super().__init__(scope, construct_id, **kwargs)

        api_name = "firecrawl-gw"
        log_group = logs.LogGroup(
            self, "FirecrawlApiGatewayLogGroup",
            log_group_name=f"/aws/apigateway/{api_name}",
            retention=logs.RetentionDays.ONE_MONTH,
            removal_policy=RemovalPolicy.DESTROY
        )

        # Firecrawl専用API Gateway
        self.rest_api = apigateway.RestApi(
            self, "FirecrawlApiGateway",
        ...
        deployment = apigateway.Deployment(
            self, "FirecrawlApiDeployment",
            api=self.rest_api
        )

        # devステージの作成
        dev_stage = apigateway.Stage(
            self, "FirecrawlDevStage",
        ...
        # API Gatewayの出力
        CfnOutput(
            self, "FirecrawlApiGatewayUrl",
            value=f"https://{self.rest_api.rest_api_id}.execute-api.{self.region}.amazonaws.com",
            description="Firecrawl API Gateway base URL"
        )

        CfnOutput(
            self, "FirecrawlApiGatewayRestApiId",
            value=self.rest_api.rest_api_id,
            description="Firecrawl API Gateway REST API ID"
        )

        CfnOutput(
            self, "FirecrawlApiGatewayDevUrl",
            value=dev_stage.url_for_path("/"),
            description="Firecrawl API Gateway dev stage URL"
        )

        CfnOutput(
            self, "FirecrawlApiGatewayProdUrl",
            value=self.prod_stage.url_for_path("/"),
            description="Firecrawl API Gateway prod stage URL"
        )